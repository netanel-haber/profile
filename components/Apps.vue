<template>
    <div>
        ...
        <section v-if="activeApp">
            <iframe id="app" :key="activeApp.hash" class="w-full my-4 border" allow="clipboard-write"
                :style="{ height: activeApp.height }" :src="activeApp.href"></iframe>
        </section>
    </div>
</template>
...
if (new Set(apps.map(a => a.hash)).size < apps.length)
    throw new Error("App hashes must be unique")
